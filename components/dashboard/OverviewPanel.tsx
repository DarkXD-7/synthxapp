...
export default function OverviewPanel({ guildId, isPremium, isOwner, settings, onRefresh }: Props) {
  ...
  // Safe data extraction — no runtime crashes
  const guild      = settings.guild      as Record<string, unknown> | null ?? null;
  const serverStats= settings.serverStats as ServerStats | null ?? null;
  const setup      = settings.setup      as Record<string, unknown> | null ?? null;
  const prefix     = (setup?.prefix as string) || "s!";

  const guildIcon  = guild?.icon as string | null ?? null;
  const guildName  = guild?.name as string ?? `Server ${guildId}`;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-5 flex items-center gap-4">
        ...
        <div className="w-16 h-16 rounded-2xl overflow-hidden flex-shrink-0 bg-[#1a1a1a] flex items-center justify-center">
          {guildIcon ? (
            <img
              src={`https://cdn.discordapp.com/icons/${guildId}/${guildIcon}.${guildIcon.startsWith("a_") ? "gif" : "png"}?size=128`}
              alt={guildName}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-600 mt-0.5 font-mono">{guildId}</p>
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            <span className="text-xs text-gray-500">
              Prefix: <span className="text-red-400 font-bold font-mono">{prefix}</span>
            </span>
      ...
      {isPremium ? (
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl card">
          ...
          <a href="https://discord.gg/WKX5HHPmWz" target="_blank" rel="noopener noreferrer" className="ml-auto btn-primary text-xs px-3 py-2 whitespace-nowrap">
            Get Premium
          </a>
        </div>
      )}
        ...
        <div className="card divide-y divide-[#1e1e1e]">
          {(() => {
            ...
            const rows = [
              ...
              an ? { label: "Anti-Nuke", value: an.enabled ? "Enabled" : "Disabled", ok: Boolean(an.enabled) } : null,
              am ? { label: "AutoMod",   value: am.enabled ? `On · ${((am.activeEvents as string[]) || []).length} filter(s)` : "Disabled", ok: Boolean(am.enabled) } : null,
              wc ? { label: "Welcome",   value: wc.enabled ? "Enabled" : "Disabled", ok: Boolean(wc.enabled) } : null,
              lv ? { label: "Leveling",  value: isPremium ? (lv.enabled ? `On · ${lv.xpPerMessage ?? 20}xp/msg` : "Disabled") : "Premium only", ok: Boolean(lv.enabled) } : null,
              ar ? { label: "Auto Role", value: ((ar.humanRoles as string[]) || []).length > 0 ? `${((ar.humanRoles as string[]) || []).length} role(s)` : "Not set", ok: ((ar.humanRoles as string[]) || []).length > 0 } : null,
              tk ? { label: "Tickets",   value: tk.enabled ? `On · ${((tk.categories as unknown[]) || []).length} categor${((tk.categories as unknown[]) || []).length === 1 ? "y" : "ies"}` : "Disabled", ok: Boolean(tk.enabled) } : null,
              nm ? { label: "Night Mode", value: nm.enabled ? "Active" : "Inactive", ok: Boolean(nm.enabled) } : null,
            ].filter(Boolean) as { label: string; value: string; ok: boolean }[];

            if (rows.length === 0) {
              return <div className="p-4 text-sm text-gray-600">No modules configured yet.</div>;
            }
            return rows.map((row) => (
